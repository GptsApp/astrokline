...
export async function POST(req: Request) {
  try {
    ...
    if (!date || !dimension) {
      return NextResponse.json({ error: 'Missing date or dimension parameter' }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ error: 'Failed to format astrology data from AI.' }, { status: 500 });
    }
