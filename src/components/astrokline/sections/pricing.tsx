...
const plans: Plan[] = [
  ...
  },
  {
    ...
    period: '/mo',
    description: 'Unlock your full reading — all 4 life dimensions.',
    features: [
      { text: 'Click any year for full tooltip details', highlight: 'full tooltip' },
      { text: 'Career, Wealth, Love & Health AI reading', highlight: '4 dimensions' },
      { text: '5 chart generations per month', highlight: '5' },
      { text: 'Save & download reports', highlight: 'Save' },
      { text: 'Personal dashboard + history' },
      { text: 'Yearly energy forecasts & life stage labels' },
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex flex-wrap items-center justify-center gap-3">
        {icons.map((icon) => (
          <div
            key={icon.name}
            className="flex h-10 w-16 items-center justify-center  bg-white"
          >
            <img
              src={icon.src}
              alt={icon.name}
    ...
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6">
        ...
        <div className="mb-16 text-center">
          ...
          {/* ── Pill Switcher ── */}
          <div className="inline-flex items-center border border-white/10 bg-white/[0.06] p-1 whitespace-nowrap backdrop-blur-sm">
            <button
              type="button"
              onClick={() => setIsAnnual(false)}
              className={`relative flex items-center gap-2 px-6 py-1.5 text-sm font-semibold transition-all duration-300 ${
                !isAnnual
            ...
            >
              <span
                className={`flex h-4 w-4 shrink-0 items-center justify-center ${!isAnnual ? 'bg-primary' : 'border-muted-foreground/40 border'}`}
              >
                {!isAnnual && (
              ...
              >
                ...
                {isPro && (
                  <div className="absolute -top-3 right-6 z-20">
                    <div className="-full bg-gradient-to-r from-red-500 to-orange-500 px-4 py-1.5 text-xs font-bold text-white shadow-lg shadow-red-500/25">
                      🔥 Limited Offer
                    </div>
                  </div>
                )}
                ...
                {isLite && (
                  <div className="absolute -top-3 left-1/2 z-20 -translate-x-1/2">
                    <div className="-full bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-1.5 text-xs font-bold text-white shadow-lg shadow-emerald-500/25">
                      ⭐ Most Popular
                    </div>
                  </div>
                )}
                ...
                {isPro && (
                  <div className="absolute -top-12 left-0 right-0 z-30 flex justify-center w-full">
                    <div className="animate-pulse whitespace-nowrap  border border-rose-500/30 bg-rose-500/10 px-3 py-1.5 text-[11px] font-semibold text-rose-200 backdrop-blur-md shadow-[0_0_15px_rgba(244,63,94,0.15)]">
                ...
                <div
                  className={`relative z-10 flex h-full flex-col  p-8 transition-all duration-300 ${
                    isPro
                      ? 'bg-[#0f0d14]'
                      : isLite
                ...
                >
                  ...
                  {isFree ? (
                    <Button
                      ...
                      className="text-foreground mb-8 h-12 w-full  border border-white/[0.08] bg-white/[0.08] font-semibold transition-all hover:bg-white/[0.14]"
                    >
                      {plan.cta}
                    </Button>
                  ) : isPro ? (
                    <Button
                      ...
                      disabled={isLoading}
                      className="bg-primary text-primary-foreground hover:bg-primary/90 mb-8 flex h-12 w-full items-center justify-center gap-2  border-0 font-semibold shadow-[0_0_20px_-5px_var(--primary)] transition-all"
                    >
                    ...
                    <Button
                      ...
                      disabled={isLoading}
                      className="mb-8 flex h-12 w-full items-center justify-center gap-2  border-0 bg-emerald-600 font-semibold text-white shadow-[0_0_20px_-5px_rgba(16,185,129,0.4)] transition-all hover:bg-emerald-500"
                    >
                      {isLoading && selectedPlan === currentPlanId(plan) && (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      )}
                    ...
                    <Button
                      ...
                      disabled={isLoading}
                      className="text-foreground mb-8 flex h-12 w-full items-center justify-center gap-2  border border-white/[0.08] bg-white/[0.08] font-semibold transition-all hover:bg-white/[0.14]"
                    >
                      {isLoading && selectedPlan === currentPlanId(plan) && (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      )}
