  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      ...
      <Input
        ...
        className="focus-visible:ring-primary/50 h-11 w-full rounded-xl border-white/10 bg-black/50 pl-9 text-white"
        autoComplete="off"
      />

      {isOpen && (results.length > 0 || isLoading) && (
        <div className="absolute top-[calc(100%+8px)] left-0 z-[100] w-full min-w-[240px] overflow-hidden rounded-xl border border-white/10 bg-[#15131A] p-2 shadow-2xl backdrop-blur-xl">
          {isLoading ? (
            ...
            <ul className="hidden-scrollbar max-h-60 overflow-y-auto">
              {results.map((loc) => {
                ...
                return (
                  <li
                    key={loc.place_id}
                    className="mb-1 flex cursor-pointer flex-col rounded-lg px-3 py-2 text-sm text-white/80 transition-colors last:mb-0 hover:bg-white/5"
                    onClick={() => {
