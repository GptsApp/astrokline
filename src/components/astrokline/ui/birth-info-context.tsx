...
export function getSavedBirthData(): BirthData | null {
  ...
  try {
    ...
    if (parsed.name && parsed.date && parsed.location && parsed.lat !== null && parsed.lon !== null) {
      return parsed;
    }
    return null;
  } catch {
...
export function saveKlineResult(result: any) {
  try {
    localStorage.setItem(KLINE_RESULT_KEY, JSON.stringify(result));
  } catch { /* ignore quota errors */ }
}
...
export function getSavedKlineResult(): any | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = localStorage.getItem(KLINE_RESULT_KEY);
    return raw ? JSON.parse(raw) : null;
  } catch { return null; }
}

export function clearSavedKlineResult() {
  try { localStorage.removeItem(KLINE_RESULT_KEY); } catch {}
}
...
export function BirthInfoProvider({ children }: { children: React.ReactNode }) {
  ...
  const [onCompleteCallback, setOnCompleteCallback] = useState<((data: BirthData) => void) | null>(null);

  const isComplete = !!(data.name && data.gender && data.date && data.timeSlot && data.location);
  ...
  return (
    <BirthInfoContext.Provider value={{ data, setData, isComplete, isModalOpen, openModal, closeModal, onCompleteCallback }}>
      {children}
...
export function useBirthInfo() {
  ...
  if (!ctx) throw new Error("useBirthInfo must be used within BirthInfoProvider");
  return { data: ctx.data, isComplete: ctx.isComplete };
}

export function useBirthInfoModal() {
  const ctx = useContext(BirthInfoContext);
  if (!ctx) throw new Error("useBirthInfoModal must be used within BirthInfoProvider");
  return {
    open: ctx.openModal,
