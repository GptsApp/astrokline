...
const DASHA_BAR_COLOR_CLASSES: Record<string, string> = {
  Sun: 'bg-[#fbbf24]',
  Moon: 'bg-[#e2e8f0]',
  Mars: 'bg-[#f87171]',
  Rahu: 'bg-[#a78bfa]',
...
const DASHA_WIDTH_CLASSES: Record<number, string> = {
  1: 'basis-[1%]',
  2: 'basis-[2%]',
  3: 'basis-[3%]',
  4: 'basis-[4%]',
...
type Props = {
  ...
  profileName?: string;
  // eslint-disable-next-line no-unused-vars
  onActionGate?: (...args: [string, AppTier]) => void;
  aiYearInsights?: Record<number, { aiSummary: string; aiAdvice: string }>;
  /** Hide the header (title, subtitle, legend, score) */
  showHeader?: boolean;
  /** Hide the Mahadasha dasha strip */
  showDasha?: boolean;
  /** Compact mode: shorter chart height, tighter margins */
  compact?: boolean;
};
...
export function InteractiveChart({
  ...
  useEffect(() => {
    ...
    const animationFrameId = window.requestAnimationFrame(updateChartReadiness);
    ...
    >
      ...
      >
        ...
        <div className="w-full overflow-visible">
          <div ref={chartContainerRef} className={cn("overflow-visible", compact ? "h-[320px]" : "h-[480px] md:h-[550px]")}>
            {isChartReady ? (
              <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={compact ? 200 : 320}>
                <ComposedChart
                  data={chartData}
                  margin={compact ? { top: 20, right: 16, left: 0, bottom: 30 } : { top: 76, right: 26, left: 5, bottom: 52 }}
                  barCategoryGap={0}
                  barGap={0}
                  onClick={(state) => {
              ...
              <div
                aria-hidden="true"
                className="h-full w-full rounded-[28px] border border-white/6 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),rgba(255,255,255,0.01))]"
              />
            )}
          </div>
        </div>

        {/* Dasha Period Indicator Bar (TradingView-style, below chart) */}
        {showDasha && tier === 'PRO' && (
          <div className="-mt-2 ml-[35px] mr-[26px] px-1">
            <p className="mb-1 font-mono text-[9px] uppercase tracking-[0.15em] text-white/30">
              Mahadasha Periods
            </p>
            <div className="flex h-5 w-full overflow-hidden rounded-sm">
              {dashaStrip
                .filter(ds => ds.endAge > 0 && ds.startAge < 100)
                .map(ds => {
                  const start = Math.max(ds.startAge, 0);
                  const end = Math.min(ds.endAge, 100);
                  const pct = end - start; // out of 100
                  const label = pct >= 10 ? ds.planet : pct >= 5 ? ds.planet.slice(0, 2) : ds.planet[0];
                  return (
                    <div
                      key={`bar-${ds.planet}`}
                      className={cn(
                        'relative flex items-center justify-center border-r border-black/40 last:border-r-0',
                        DASHA_WIDTH_CLASSES[Math.round(pct)] ?? 'flex-1',
                        DASHA_BAR_COLOR_CLASSES[ds.planet] ?? 'bg-white/10'
                      )}
                      title={`${ds.planet}: age ${start}–${end}`}
                    >
                      <span className="truncate px-0.5 font-mono text-[7px] font-bold text-black/70">
                        {label}
                      </span>
                    </div>
                  );
...
const CandleTooltip = ({ active, payload, transitDetails, tier, onActionGate, aiYearInsights }: any) => {
  ...
  // Detect viewport overflow and flip tooltip to left side
  useEffect(() => {
    ...
    if (rect.right > vw - 8) {
      // Shift left by the overflow amount + small gap
      el.style.transform = `translateX(-${Math.ceil(rect.right - vw + 16)}px)`;
    } else if (rect.left < 8) {
      el.style.transform = `translateX(${Math.ceil(8 - rect.left)}px)`;
    } else {
      el.style.transform = '';
    }
  });
  ...
  return (
    <div ref={wrapperRef} className="relative z-40 max-w-[calc(100vw-24px)] min-w-[280px] border border-white/10 bg-[#050505] shadow-2xl sm:max-w-[420px] sm:min-w-[340px]">
