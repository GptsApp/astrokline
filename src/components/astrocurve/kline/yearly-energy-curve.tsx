...
export function YearlyEnergyCurve({ klineData, tier, onActionGate }: Props) {
  ...
  const [monthlyData, setMonthlyData] = useState<ReturnType<typeof getMonthlyEnergies> | null>(null);
  ...
  const maxScore = monthlyData ? Math.max(...monthlyData.map(m => m.score)) : 0;
  const minScore = monthlyData ? Math.min(...monthlyData.map(m => m.score)) : 0;
