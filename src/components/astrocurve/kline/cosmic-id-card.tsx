...
export function CosmicIdCard({ profile, klineData }: Props) {
  ...
  const [shareUrl, setShareUrl] = useState<string>('https://www.astrocurve.net/kline');
  ...
  useEffect(() => {
    runClientRequest(
      KLINE_REFERRAL_CACHE_KEY,
      async () => {
        const res = await fetch('/api/kline/referral');
        return res.json();
      },
      { ttlMs: 30_000 }
    )
    ...
    <AnimatePresence>
      ...
      >
        <button 
          ...
          aria-label="Close cosmic card"
          title="Close cosmic card"
