...
export function EnergyTool({ tier, klineResult }: EnergyToolProps) {
  ...
  // Mark onboarding step complete when user visits energy tool
  useEffect(() => {
    import('@/components/astrocurve/dashboard/onboarding-guide').then(m => m.completeOnboardingStep('check_energy'));
  }, []);
